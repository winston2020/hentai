...
@section('title','电影区-'.$tag->name.'-'.$tdk->title)
                ...
                <div id="content" class="col-md-12" role="main">
                    <section class="video-listing">
                        <div class="video-listing-head">
                            <h2 class="light-title">{{$tag->name}}</h2>
                                        ...
                                        <div id="post-41287" class="video-item post-41287 post type-post status-publish format-video has-post-thumbnail hentry category-bangumi tag-9405 tag-9162 post_format-post-format-video">
                                            <div class="qv_tooltip tooltipstered">
                                                <div class="item-thumbnail">
                                                    <a href="{{url('bangumi')}}/{{$value['id']}}" > <img data-original="{{$value['video_cover_img_url']}}" src="{{$value['video_cover_img_url']}}" alt="{{$value['name']}}" title="{{$value['name']}}">
                                            ...
                                            <div class="item-head">
                                                ...
                                                <div class="item-info hidden"> <span class="item-author"><a href="{{url('u')}}/{{$value['userid']}}" title="由{{\App\User::find($value['userid'])->name}}发布" rel="author">{{\App\User::find($value['userid'])->name}}</a></span> <span class="item-date">{{$value['created_at']}}</span>
